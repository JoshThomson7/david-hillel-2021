...
(function ($, root, undefined) {
    ...
    $(window).on('load', function () {
        ...
        var deptDefault = $('#apf_search').data('dept-default');

        if(deptDefault) {
            el = 'input[value="'+deptDefault+'"]';
            $(el).trigger('click');
        }
    ...
    function fetchPropertiesOnLoad() {
        ...
        if($('.apf:not(.apf__book__viewing__thanks) .apf__results').length > 0) {
    ...
    function handleSoldLetText() {
        ...
        switch (type) {
            case 'to-let':
                text = 'Hide Let, Let Agreed'
                break;
        
            default:
                var text = 'Hide Sold, Sold STC & Under offer';
                break;
        }
